...
void D02Z01_2015::ObliczZadanie()
{
...
int D02Z01_2015::ObliczIloscPapieru(std::smatch dlugosci)
{
	int boki[] = { std::stoi(dlugosci[1]), std::stoi(dlugosci[2]), std::stoi(dlugosci[3])};
	std::sort(std::begin(boki), std::end(boki));

	return 3 * boki[0] * boki[1] + 2 * boki[1] * boki[2] + 2 * boki[2] * boki[0];
}

D02Z01_2015::D02Z01_2015(std::string sciezka)
{
...
D02Z01_2015::~D02Z01_2015()
{
	this->_SciezkaDoPliku = "";
	this->_Wynik = 0;
}

int D02Z01_2015::PokazWynik()
{
	return this->_Wynik;
}
